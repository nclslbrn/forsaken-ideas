...
export default class SvgTracer {
    ...
    init () {
        if (this.parentElem && this.width && this.height) {
            ...
            this.elem.setAttribute('viewBox', `0 0 ${this.width} ${this.height}`)
            this.elem.setAttribute('background', this.background)
            this.elem.setAttribute('style', `background: ${this.background}; box-shadow: 0 0.5em 1em rgba(0,0,0,0.1);`)
            this.elem.style.aspectRatio = `${this.width} / ${this.height}`

            // create an array of group instance (key = group(props.name))
            this.groups = []
            this.parentElem.appendChild(this.elem)

            this.rect({x: 0, y: 0, w: this.width, h: this.height, fill: this.background})
        }
    }
    /**
