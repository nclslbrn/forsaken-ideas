...
vec4 map(vec3 p) {
    float d = sdCube(p - vec3(0, 0, -.25), vec3(5., 5., .5));
    int hitCell = -1;
    ...
    for (int i = 0; i < MAX_CELLS; i++) {
        ...
        vec3 size = vec3(c.z * 0.97, c.w * 0.97, .66); // thickness

        float dBox = sdCube(p - center, size * 0.25);
    ...
    return vec4(d, 1.0, 1.0, float(hitCell));
}
...
void main() {
    ...
    float attenuationFactor = float(u_lightCount) * 12.;

    if (tmat.x > 0.) {
        vec3 pos = rayOrigin + tmat.x * rayDir;
        vec3 nor = calcNormal(pos);
        vec3 matcol = tmat.w < 0.
            ? vec3(0.8)
            : vec3(1.);
        ...
        for (int i = 0; i < MAX_LIGHTS; i++) {
            ...
            float attenuation = 1.0 / (1.0 + lightDist * lightDist * attenuationFactor);
