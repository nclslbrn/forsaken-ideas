...
const sketch = (p5) => {
    ...
    const randPos = (margin = 0) => [
        p5.random(margin, 1 - margin) * p5.width,
        p5.random(margin, 1 - margin) * p5.height
    ]
    ...
    const scribbleSquare = (x, y, w) => {
        ...
        points.map((line) => {
            line.forEach((pt) =>
                p5.vertex(pt[0] + p5.random(-3, 3), pt[1] + p5.random(-3, 3))
            )
        })
    ...
    p5.draw = () => {
        ...
        p5.beginShape()
        p5.stroke(180)
        p5.vertex(ellPoints[0].x, ellPoints[0].y)
    ...
    sketch.init = () => p5.redraw()
    sketch.download = () => p5.save('In-the-style-of-Hilma-Af-Klint')
}
