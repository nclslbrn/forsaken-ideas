...
const MAX_STEPS = 1000,
    ...
    { PI, max, min, abs, floor, pow } = Math

const rotateAll = (p) => {
    let rotated = rotateX(p, 0.2)
...
const getNormal = (p) => {
    ...
    return vec3.normalize([
        map([p[0] + eps, p[1], p[2]], ITERATIONS) -
            map([p[0] - eps, p[1], p[2]], ITERATIONS),
        map([p[0], p[1] + eps, p[2]], ITERATIONS) -
            map([p[0], p[1] - eps, p[2]], ITERATIONS),
        map([p[0], p[1], p[2] + eps], ITERATIONS) -
            map([p[0], p[1], p[2] - eps], ITERATIONS)
    ])
}
