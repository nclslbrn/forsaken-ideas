...
import { random, ceil, abs, sqrt, cos, sin, atan2, round } from '../../sketch-common/Math'
...
const noise = (x, y) => {
  const freq = 0.03
  const turbulence = 20
  return turbulence * noise2D(x * freq, y * freq)
}
...
const sketch = {
  ...
  init: () => {
    ...
    for (let i = 0; i < circleNum - 1; i++) {
      radiuses.push((1 / circleNum) * (i + 1) * (svg.width - margin * 2) * 0.7)
    }
    if (debug) {
      sketch.drawFrame()
      sketch.drawRadiuses()
    ...
    for (let x = frame.min.x; x <= frame.max.x; x += pointSpacing) {
      for (let y = frame.min.y; y <= frame.max.y; y += pointSpacing) {
        points.push(new Point(x, y, atan2(svg.height / 2 - y, svg.width / 2 - x)))
        lines.push([])
      }
    }
      ...
      for (let i = 0; i < points.length; i++) {
        if (!points[i].isStuck) {
          ...
          if (sketch.getRadiusNum(pos) % 2 == 0) {
            ...
            const curvePos = funcs[planeCurve](p, 5)
            const loopPos = funcs['sinusoidal'](curvePos)
            points[i].angle = noise(loopPos.x, loopPos.y)
          } else {
            points[i].angle = noise(round(pos[0] / 64) * 16, round(pos[1] / 64) * 16)
          }
          points[i].pos = [
    ...
    new Notification(`Sketch drawn with ${planeCurve.replaceAll('_', ' ')}.`, container, 'light')
  },
  getRadiusNum: (pt) => {
  ...
  export: () => {
    svg.export({ name: 'concentric-wave' })
  }
}
