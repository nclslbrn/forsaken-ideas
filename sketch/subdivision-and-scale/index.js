...
const capture = (canvas) => {
  ...
  link.download = `Variationes_in_triangulis__std_IV_Nicolas_Lebrun.jpg`;
...
const sketch = {
  init: () => {
    const numCell = 1 + ceil(random() * 16);
  ...
  exportSvg: () => {
    const polygons = [
      ...fillWithStraightLines(canvas, (c) => c < 128, 4, Math.random() > .5 ? 2 : 3),
      ...fillWithStraightLines(canvas, (c) => c < 128, 4, Math.random() > .5 ? 1 : 0).filter((line, idx) => idx % 6 !== 0),
