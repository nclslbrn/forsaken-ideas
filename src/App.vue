...
export default defineComponent({
        ...
        fetch(`sketch/index.json?cache=${isoDate()}`)
            ...
            .then((data) => {
                ...
                if (window.location.hash.substring(1)) {
                    ...
                    this.prevClicked = this.projects.indexOf(
    ...
    methods: {
        ...
        onWheel(event: WheelEvent) {
            this.$nextTick(
                () =>
                    ((this.$refs.theWall as HTMLElement).scrollLeft +=
                        event.deltaY)
            )
        },
        /**
        ...
        async addObserver() {
            ...
            const callback: IntersectionObserverCallback = (entries: any) => {
                if (!this.navButtonOccurs) {
                    // prevent selecting back slide when nav button clicked
...
<template>
    <main>
        ...
        <div class="row">
            <ProjectCaption
                v-if="projects[currProjectIndex] !== undefined"
                :project="projects[currProjectIndex]"
                @openProject="openProject"
            />
            ...
            <div class="ui">
                <nav>
                    <ul>
                        <li>
                            <button class="prevProj" @click="prevProj">
                                &#60;
                            </button>
                        </li>
                        <li>
                            <label class="projectIdx"
                                >{{ currProjectIndex + 1 }} /
                                {{ projects.length }}</label
                            ><small>projects</small>
                        </li>
                        <li>
                            <button class="prevProj" @click="nextProj">
                                &#62;
                            </button>
                        </li>
                    </ul>
