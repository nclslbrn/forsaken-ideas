...
export default async function (path) {
  ...
  const rowLibs = await input({
    message: 'Library loaded by CDN you can also use files from sketch-common/' +
  ...
  const info = await input({
    message: 'write a short sketch description, or leave it blank (and we fill it with "TODO") :'
  })
  ...
  const topic = await input({
    message: 'Topic/theme for this sketch, available topics: ' +
  ...
  if (actionsConcated.indexOf('=') > -1) {
    if (actionsConcated.indexOf(',') > -1) {
      actions = actionsConcated.split(',').map(item => item.replace(' ', ''))
    } else {
  ...
  props.date = today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + today.getDate()

  console.log(props)
  const rewiewed = await confirm({ message: 'Does this configuration seem correct ?' })

  if (rewiewed) {
    const jsonContent = JSON.stringify(props)
    fs.writeFileSync(`${path}/property.json`, jsonContent, 'utf-8', (err) => {
      if (err) {
        throw new Error(err)
      } else {
        console.log(`Your sketch preset are saved on ${path}/property.json`)
      }
    })
  }
}